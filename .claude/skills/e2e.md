...
- Which OSAC components are involved (fulfillment-service, operator, AAP)
...
If the user provided a Jira ticket key in Step 1, use it. Otherwise, ask: "What Jira ticket key should I use for the commit? (or type NO-ISSUE)"
...
**K8s CRD shortnames:** `computeinstance`, `virtualnetwork`, `subnet`, `securitygroup`, `clusterorder`, `publicip`, `publicippool`, `tenant`
